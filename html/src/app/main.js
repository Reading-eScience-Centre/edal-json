...
interpolationSwitcher.onAdd = function (map) {
  ...
  $('.interpolation-button', div).click(function () {
    map.interpolation = interpolation[$(this).data('interpolation')]
    ...
    function drawAnyProjection () {
      ...
      for (let tileX = 0; tileX < tileSize; tileX++) {
        for (let tileY = 0; tileY < tileSize; tileY++) {
          ...
          // var t0 = Date.now()
          let iLatNeighbors = indicesOfNearest(domainLat, lat)
          let iLonNeighbors = indicesOfNearest(domainLon, lon)
          // tNear += Date.now()-t0
